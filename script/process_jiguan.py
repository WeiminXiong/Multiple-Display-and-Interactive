...
def process_xian_data(data):
    si, fu, data_item = data
    if len(data_item) == 3:
        xian = data_item[:2]
...
save_path = "./js/data.js"
with open(data_path, 'r', encoding='utf-8') as f:
    data = json.load(f)

for item in data:
    si,fu,xian = process_xian_data(process_fu_data(process_si_data(item)))
    item['司'] = si
    item['府'] = fu
    item['县'] = xian
